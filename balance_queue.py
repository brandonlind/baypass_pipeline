...
# as run in pipeline when jobs from 01_start_baypass_pipeline.py:
...
import os, shutil, sys, math, subprocess, time, pickle
...
#                 inp = input(Bcolors.WARNING + "\tINPUT NEEDED: Do you want to use this account: %s? (yes | no): " % acct + Bcolors.ENDC).lower()
                inp = input(ColorText("\tINPUT NEEDED: Do you want to use this account: %s? (yes | no): " % acct).warn().__str__()).lower()
                if inp in ['yes', 'no']:
                    if inp == 'yes':
    ...
    while len(keep) == 0:
        print(ColorText("FAIL: You need to specify at least one account. Revisiting accounts...").fail())
...
def get_avail_accounts(parentdir=None, save=False):
    ...
    # save if necessary
    if save is True:
        # called from 00_start_baypass_pipeline.py
        keep = choose_accounts(accts)
